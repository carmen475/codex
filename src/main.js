...
const products = [
  {
    ...
    description: "Replace this placeholder with a real product from your store.",
  },
  {
    id: "sample-two",
    ...
    description: "Use this card for another product, service, or package.",
  },
  {
    id: "sample-three",
    ...
    description: "Edit the product list in src/main.js when your real catalog is ready.",
  },
];
...
function cartItems() {
  return products.filter((p) => state.cart[p.id]).map((p) => ({ ...p, qty: state.cart[p.id] }));
}

function totals() {
  const subtotal = cartItems().reduce((sum, item) => sum + item.price * item.qty, 0);
  const shipping = subtotal ? 9 : 0;
...
function summary() {
  ...
  return `
    <dl class="summary">
      ...
      <div><dt>Shipping</dt><dd>${money.format(t.shipping)}</dd></div>
      <div><dt>Tax</dt><dd>${money.format(t.tax)}</dd></div>
...
function productList() {
  return `
    <section class="products" aria-labelledby="products-title">
      <div class="heading">
        <p>Catalog</p>
        <h2 id="products-title">Placeholder products</h2>
      </div>
      <div class="grid">
        ${products
          .map(
...
function cartPanel() {
  ...
  return `
    <section class="panel" aria-labelledby="cart-title">
      ...
      ${summary()}
      <button class="primary full" data-action="details" ${items.length ? "" : "disabled"}>Continue to details</button>
    </section>
  `;
}
...
function detailsForm() {
  ...
  return `
    <form class="panel form" data-form="details" novalidate>
      <div class="heading"><p>Step 2</p><h2>Customer details</h2></div>
      ...
      <label>Address<input name="address" value="${escapeHtml(c.address)}" ${attrsFor("address")} />${errorFor("address")}</label>
      <div class="row">
  ...
  return `
    <form class="panel form" data-form="payment" novalidate>
      ...
      <div class="actions">
        ...
        <button class="primary" type="submit">Place test order</button>
      </div>
    </form>
  `;
}
...
function resultPanel(success) {
  return `
    <section class="panel result ${success ? "" : "failed"}">
      <p class="badge">${success ? "Order created" : "Payment declined"}</p>
      <h2>${success ? "Checkout completed safely" : "Test payment failed"}</h2>
      <p>${success ? `Order ${state.order.id} was created. No real charge was made.` : "The simulated bank declined this test card. Try the success card to finish the flow."}</p>
      ${success ? summary() : ""}
...
function render() {
  ...
  app.innerHTML = `
    <header>
      <div><p>Replace with your store name</p><h1>Checkout Starter</h1></div>
      <button class="cart" data-action="cart">Cart <span>${count}</span></button>
    </header>
    <main>
      <section class="hero">
        <p>Safe checkout demo</p>
        <h2>A starter checkout flow you can customize for the real store.</h2>
        <p>Products, cart edits, customer validation, simulated payment success, and simulated decline are included.</p>
      </section>
      <div class="layout">
        ${productList()}
        ${activePanel()}
